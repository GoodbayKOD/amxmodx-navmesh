...
public plugin_init()
{
    ...
    // Try load
    Navmesh_LoadInternal(szCurrentMap);*/
}
...
public bool:native_build_path(plugin_id, num_params)
{
    ...
    if(iStartArea == INVALID_NAV_AREA || iGoalArea == INVALID_NAV_AREA)
        return false;
...
public bool:native_get_path_segment(plugin_id, num_params)
{
    ...
    new Float:vPos[3];
    vPos[0] = sSegment[NAV_PATH_POS_X];
    vPos[1] = sSegment[NAV_PATH_POS_Y];
    vPos[2] = sSegment[NAV_PATH_POS_Z];
...
bool:LoadArea(iFile, iVersion)
{
    ...
    if(!FileReadFloat(iFile, fNEZ))
        return false;
    if(!FileReadFloat(iFile, fSWZ))
        return false;
    ...
    for(new NavDirType:dir = NAV_DIR_NORTH; dir < NUM_NAV_DIRECTIONS; dir++)
    {
        ...
        for(new i = 0; i < iConnectCount; i++)
        {
            ...
            sConnect[NAV_CONNECT_AREA] = INVALID_NAV_AREA; // Will be resolved in PostLoad
    ...
    for(new i = 0; i < sArea[NAV_AREA_APPROACH_COUNT]; i++)
    {
        ...
        if(!FileReadInt32(iFile, sApproach[NAV_APPROACH_HERE_ID]))
            return false;
        if(!FileReadInt32(iFile, sApproach[NAV_APPROACH_PREV_ID]))
            return false;
        ...
        if(!FileReadInt32(iFile, sApproach[NAV_APPROACH_NEXT_ID]))
            return false;
        if(!FileReadUint8(iFile, iType))
            return false;
    ...
    sArea[NAV_AREA_PARENT] = INVALID_NAV_AREA;
    sArea[NAV_AREA_PARENT_HOW] = NAV_TRAVERSE_NORTH;
    sArea[NAV_AREA_MARKER] = 0;
    sArea[NAV_AREA_OPEN_MARKER] = 0;
    sArea[NAV_AREA_PREV_HASH] = INVALID_NAV_AREA;
    sArea[NAV_AREA_NEXT_HASH] = INVALID_NAV_AREA;
...
bool:PostLoadArea(iAreaIndex)
{
    ...
    for(new NavDirType:dir = NAV_DIR_NORTH; dir < NUM_NAV_DIRECTIONS; dir++)
    {
        ...
        for(new i = 0; i < ArraySize(aConnect); i++)
        {
            ...
            if(iConnectAreaIndex == INVALID_NAV_AREA)
            {
...
InitGrid(Float:fMinX, Float:fMaxX, Float:fMinY, Float:fMaxY)
{
    ...
    for(new i = 0; i < 256; i++)
        g_nGrid[NAV_GRID_HASH_TABLE][i] = INVALID_NAV_AREA;
}
...
AddAreaToGrid(iAreaIndex)
{
    ...
    if(g_nGrid[NAV_GRID_HASH_TABLE][iHash] != INVALID_NAV_AREA)
    {
        new sOther[NavArea];
...
RemoveAreaFromGrid(iAreaIndex)
{
    ...
    // Update links
    if(sArea[NAV_AREA_PREV_HASH] != INVALID_NAV_AREA)
    {
        new sPrev[NavArea];
    ...
    if(sArea[NAV_AREA_NEXT_HASH] != INVALID_NAV_AREA)
    {
        new sNext[NavArea];
...
FindAreaByIDFast(iAreaID)
{
    ...
    while(iAreaIndex != INVALID_NAV_AREA)
    {
        new sArea[NavArea];
        ArrayGetArray(g_aNavAreas, iAreaIndex, sArea);
...
Navmesh_GetNearestAreaInternal(const Float:vOrigin[3], Float:fMaxDist, Float:fBeneathLimit)
{
    ...
    new iNearestArea = INVALID_NAV_AREA;
...
bool:BuildPathAStar(iStartArea, iGoalArea, Array:aPathOut)
{
    ...
    while(ArraySize(aOpenList) > 0 && iIterations < MAX_ITERATIONS)
    {
        ...
        if(iCurrentArea == INVALID_NAV_AREA)
            break;
        ...
        for(new NavDirType:dir = NAV_DIR_NORTH; dir < NUM_NAV_DIRECTIONS; dir++)
        {
            ...
            for(new i = 0; i < ArraySize(aConnect); i++)
            {
                ...
                if(iNeighborArea == INVALID_NAV_AREA)
                    continue;
                ...
                if(sNeighbor[NAV_AREA_MARKER] != iMasterMarker || fNewCost < sNeighbor[NAV_AREA_COST_SO_FAR])
                {
                    ...
                    sNeighbor[NAV_AREA_TOTAL_COST] = fNewCost + GetHeuristicCost(iNeighborArea, iGoalArea);
    ...
    while(iCurrentArea != INVALID_NAV_AREA && iCurrentArea != iStartArea)
    {
...
Float:GetMoveCost(iFromArea, iToArea)
{
    ...
    new Float:fCost = GetHeuristicCost(iFromArea, iToArea);
